...
class TTLock extends TTLockApi_1.TTLockApi {
    ...
    async addPassCode(type, passCode, startDate, endDate) {
        ...
        try {
            ...
            if (error instanceof TTLockApi_1.PasscodeOperationError) {
     ...
     * @param endDate Valid to YYYYMMDDHHmm
     */
    async recoverPassCode(type, passCode, startDate, endDate) {
        if (!this.initialized) {
            throw new Error('Lock is in pairing mode');
        }
        if (!this.isConnected()) {
     ...
     *                 `skipBackfill: true`.
     */
    async getOperationLog(all = false, noCache = false, options = {}) {
        var _a, _b;
        if (!this.initialized) {
            return [];
        }
        ...
        if (all) {
            ...
            const deadline = Date.now() + ((_a = options.maxDurationMs) !== null && _a !== void 0 ? _a : 5 * 60 * 1000);
            let operations = [];
            ...
            if (operations.length == 0) {
                ...
                // sweep can terminate.
                const probeMaxConsecutiveEmpty = (_b = options.maxProbeEmpty) !== null && _b !== void 0 ? _b : 20;
...
TTLock.MAX_MISSING_SEQUENCES = 10000;
