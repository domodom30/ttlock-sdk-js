...
const log = createLogger('ttlock:comm');
const DEFAULT_HEADER = Buffer.from([0x7f, 0x5a]);

export class CommandEnvelope {
  static readonly APP_COMMAND: number = 0xaa;
  ...
  static createFromRawData(rawData: Buffer, aesKey?: Buffer): CommandEnvelope {
    ...
    if (command.protocol_type >= 5 || command.protocol_type == 0) {
      //New agreement
  ...
  private generateLockType(): void {
    if (this.protocol_type == 0x05 && this.sub_version == 0x03 && this.scene == 0x07) this.setLockType(LockType.LOCK_TYPE_V3_CAR);
    else if (this.protocol_type == 0x0a && this.sub_version == 0x01) this.setLockType(LockType.LOCK_TYPE_CAR);
    else if (this.protocol_type == 0x05 && this.sub_version == 0x03) this.setLockType(LockType.LOCK_TYPE_V3);
    else if (this.protocol_type == 0x05 && this.sub_version == 0x04) this.setLockType(LockType.LOCK_TYPE_V2S_PLUS);
    else if (this.protocol_type == 0x05 && this.sub_version == 0x01) this.setLockType(LockType.LOCK_TYPE_V2S);
    else if (this.protocol_type == 0x0b && this.sub_version == 0x01) this.setLockType(LockType.LOCK_TYPE_MOBI);
    else if (this.protocol_type == 0x03) this.setLockType(LockType.LOCK_TYPE_V2);
  }
